...
// ─────────────────────────────────────────────────────────────────────────────
// Precios regionales (3 tiers, todos USD). DEMO: se derivan de un precio base con
// un factor por región. Reemplazar por la tabla real de precios por región del
// cliente (puede ser un objeto {us,latam,es} explícito por producto).
// ─────────────────────────────────────────────────────────────────────────────
export const PRICE_TIER_FACTOR: Record<PriceTier, number> = {
  us: 1, // EE. UU.
  latam: 1.12, // LATAM (DEMO: +12%)
  es: 1.06, // España / mercado ES (DEMO: +6%)
};

/** Formatea un precio base USD según el tier regional activo. */
export function formatPrice(baseUSD: number, tier: PriceTier): string {
  const value = Math.round(baseUSD * PRICE_TIER_FACTOR[tier]);
...
export type JaltestLine = {
  ...
  priceUSD: number; // precio base (tier "us"); ver formatPrice()
  logo: string; // SVG del logo Jaltest a color
  vehicleImg?: string; // foto del vehículo (cutout); puede faltar
  kitImg: string;
  description: Localized<string[]>; // párrafos por idioma
};

// Kits de cobertura (honeycomb home). `label` es una clave del diccionario
// (coverage.labels). Imágenes reusan cutouts de vehículos.
